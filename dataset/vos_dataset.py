...
class VOSALDataset(Dataset):
    """
    Works for Ali dataset training
    For each sequence:
    - Pick three frames
    - Pick two objects
    - Apply some random transforms that are the same for all frames
    ...
    - The distance between frames is controlled
    """
    def __init__(self, data_root, train_path, val_path, max_jump, is_bl, subset=None):
        ...
        print('%d out of %d videos accepted in %s.' % (len(self.videos), len(vid_list), data_root))
